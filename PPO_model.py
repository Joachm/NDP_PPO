...
import chex
import distrax
from distreqx import distributions as dqx
...
class LogEnvState(NamedTuple):
    env_state: brax.envs.State  # Update to Brax EnvState
...
class LogWrapper(BraxWrapper):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, LogEnvState]:
        """Reset the environment and return the initial observation and LogEnvState."""
        env_state = self._env.reset(key)
        obs = env_state.obs
        
        n_envs = obs.shape[0]
        state = LogEnvState(env_state, jnp.zeros((n_envs,)), jnp.zeros((n_envs,)), jnp.zeros((n_envs,)), jnp.zeros((n_envs,)), jnp.zeros((n_envs,)))
        return obs, state
    ...
    def step(
        ...
        """Take a step in the environment and update logging."""
        # Brax step takes (env_state, action) and returns the next state
        env_state = self._env.step(state.env_state, action)
        obs = env_state.obs
        ...
        # Update the log state with episode info
        state = LogEnvState(
            env_state=env_state,
...
class PPO:
    ...
    def __init__(self, mdl_factory: Callable, env: Environment, config: Config) -> None:
        ...
        if config.anneal_lr:
            ...
            tx = optax.chain(
                optax.clip_by_global_norm(config.max_grad_norm),
                #optax.sgd(config.lr, eps=1e-5),
                optax.sgd(config.lr),
            )
        self.tx = tx
        self.eval_fn = rx.tasks._brax.BraxTask(self.cfg.env_name,1000, lambda prms: EvalWrapper(mdl_factory(prms)))
    #-------------------------------------------------------------------
    @eqx.filter_jit
    ...
    def update_step(self, train_state: TrainState, key):
        ...
        ##Sanity check with no PPO learning
        #return (train_state,
        #    [[0],eval_score]
        #)
    ...
    def init_train_state(self, params, key)->TrainState:
        ...
        return TrainState(
            ...
            obs=obs, 
            dones=jnp.zeros((self.cfg.num_envs,)))
    #-------------------------------------------------------------------
...
class ActorCritic(eqx.Module):
    ...
    discrete: bool
    # ---
    def __init__(self, obs_dims, action_dims, discrete=True, *, key):
        k1, k2 = jr.split(key)
        self.actor = nn.MLP(obs_dims, action_dims, 32, 3, key=k1, activation=jnn.tanh)
        self.critic = nn.MLP(obs_dims, "scalar",64, 2, key=jr.key(0), activation=jnn.tanh)
        self.discrete = discrete
        self.sigma = jnp.ones((action_dims,)) * 0.0
    # ---
    def __call__(self, obs, mean, variance):
        #obs = (obs - mean) / variance

        mu = self.actor(obs)
        value = self.critic(obs)
        if self.discrete:
            pi = dqx.Categorical(logits=y)
        else:
            sigma = jnp.exp(self.sigma)
            pi = NormalDiag(jnn.tanh(mu), sigma)
            #pi = NormalDiag(jnn.tanh(mu), 0.7) ##use untrainable sigma
        return pi, value
    # ---
